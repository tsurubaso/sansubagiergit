...
export default function StoriesGrid({ jsonUrl = "/stories.json", status = "fragment" }) {
  ...
  useEffect(() => {
    const fetchStories = async () => {
      try {
        ...
        // Filtrage par statut
        setStories(data.filter((story) => story.status === status));
      } catch (error) {
        ...
        {filteredStories.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {filteredStories.map((story) => (
              <Link key={story.id} href={`/${status}list/${story.link}`} passHref>
                <div
